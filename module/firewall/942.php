...
return [
  ...
  [
    ...
    'rule' => [
      ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '~(?i:sleep\s*?\(.*?\)|benchmark\s*?\(.*?\,.*?\))~'],
    ],
  ],
  [
    'id' => 942170,
    ...
    'rule' => [
      ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '~(?i)(?:select|;)[\s\x0b]+(?:benchmark|if|sleep)[\s\x0b]*?\([\s\x0b]*?\(?[\s\x0b]*?[0-9A-Z_a-z]+~'],
    ],
  ],
  [
    'id' => 942190,
  ...
  [
    ...
    'rule' => [
      ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '~^(?i:-0000023456|4294967295|4294967296|2147483648|2147483647|0000012345|-2147483648|-2147483649|0000023456|2.2250738585072007e-308|2.2250738585072011e-308|1e309)$~'],
    ],
  ],
  // [
  //   'id' => 942230,
  ...
  //   'rule' => [
  //     ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '(?i)[\s\x0b\(\)]case[\s\x0b]+when.*?then|\)[\s\x0b]*?like[\s\x0b]*?\(|select.*?having[\s\x0b]*?[^\s\x0b]+[\s\x0b]*?[^\s\x0b0-9A-Z_a-z]|if[\s\x0b]?\([0-9A-Z_a-z]+[\s\x0b]*?[<->~]'],
  //   ],
  // ],
  [
    'id' => 942240,
    ...
    'rule' => [
      ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '~(?i)alter[\s\x0b]*?[0-9A-Z_a-z]+.*?char(?:acter)?[\s\x0b]+set[\s\x0b]+[0-9A-Z_a-z]+|["\'`](?:;*?[\s\x0b]*?waitfor[\s\x0b]+(?:time|delay)[\s\x0b]+["\'`]|;.*?:[\s\x0b]*?goto)~'],
    ],
  ],
  [
    'id' => 942250,
    ...
    'rule' => [
      ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '~(?i)m(?:erge.*?using|atch[\s\x0b]*?[\(\)\+-\-0-9A-Z_a-z]+[\s\x0b]*?against)[\s\x0b]*?\(|execute[\s\x0b]*?immediate[\s\x0b]*?["\'`]~'],
    ],
  ],
  [
    'id' => 942270,
    ...
    'rule' => [
      ['w' => ['cookie', 'cookie_names', 'args', 'args_names'], 'rx' => '~(?i)union.*?select.*?from~'],
    ],
  ],
];