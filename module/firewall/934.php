...
return [
  ...
  [
    ...
    'rule' => [
      ['w' => ['cookie', 'get', 'files_name'], 'rx' => $rx_934110],
    ],
  ],
  [
    'id' => 934190,
    ...
    'msg' => 'Possible Server Side Request Forgery (SSRF) Attack: Cloud provider metadata URL in Parameter',
    'rule' => [
      ['w' => ['cookie', 'get', 'files_name'], 'rx' => $rx_934190],
    ],
  ],
  [
    'id' => 934130,
    ...
    'rule' => [
      ['w' => ['cookie', 'get'], 'rx' => '~__proto__|constructor[\s\x0b]*(?:\.|\]?\[)[\s\x0b]*prototype~'],
    ],
  ],
  [
    'id' => 934150,
    ...
    'rule' => [
      ['w' => ['cookie', 'get'], 'rx' => '~Process[\s\x0b]*\.[\s\x0b]*spawn[\s\x0b]*\(~'],
    ],
  ],
  [
    'id' => 934160,
    ...
    'rule' => [
      ['w' => ['cookie', 'get', 'files_name'], 'rx' => "~while[\s\x0b]*\([\s\x0b\(]*(?:!+(?:false|null|undefined|NaN|[\+\-]?0|\"{2}|'{2}|`{2})|(?:!!)*(?:(?:t(?:rue|his)|[\+\-]?(?:Infinity|[1-9][0-9]*)|new [A-Za-z][0-9A-Z_a-z]*|window|String|(?:Boolea|Functio)n|Object|Array)\b|\{[^\}]*\}|\[[^\]]*\]|\"[^\"]+\"|'[^']+'|`[^`]+`)).*\)~"],
    ],
  ],
  [
    'id' => 934170,
    ...
    'rule' => [
      ['w' => ['cookie', 'get', 'files_name'], 'rx' => "~^data:(?:(?:\*|[^!\"\(\),/:-\?\[-\]\{\}]+)/(?:\*|[^!\"\(\),/:-\?\[-\]\{\}]+)|\*)(?:[\s\x0b]*;[\s\x0b]*(?:charset[\s\x0b]*=[\s\x0b]*\"?(?:iso-8859-15?|utf-8|windows-1252)\b\"?|(?:[^\s\x0b-\"\(\),/:-\?\[-\]c\{\}]|c(?:[^!\"\(\),/:-\?\[-\]h\{\}]|h(?:[^!\"\(\),/:-\?\[-\]a\{\}]|a(?:[^!\"\(\),/:-\?\[-\]r\{\}]|r(?:[^!\"\(\),/:-\?\[-\]s\{\}]|s(?:[^!\"\(\),/:-\?\[-\]e\{\}]|e[^!\"\(\),/:-\?\[-\]t\{\}]))))))[^!\"\(\),/:-\?\[-\]\{\}]*[\s\x0b]*=[\s\x0b]*[^!\(\),/:-\?\[-\]\{\}]+);?)*(?:[\s\x0b]*,[\s\x0b]*(?:(?:\*|[^!\"\(\),/:-\?\[-\]\{\}]+)/(?:\*|[^!\"\(\),/:-\?\[-\]\{\}]+)|\*)(?:[\s\x0b]*;[\s\x0b]*(?:charset[\s\x0b]*=[\s\x0b]*\"?(?:iso-8859-15?|utf-8|windows-1252)\b\"?|(?:[^\s\x0b-\"\(\),/:-\?\[-\]c\{\}]|c(?:[^!\"\(\),/:-\?\[-\]h\{\}]|h(?:[^!\"\(\),/:-\?\[-\]a\{\}]|a(?:[^!\"\(\),/:-\?\[-\]r\{\}]|r(?:[^!\"\(\),/:-\?\[-\]s\{\}]|s(?:[^!\"\(\),/:-\?\[-\]e\{\}]|e[^!\"\(\),/:-\?\[-\]t\{\}]))))))[^!\"\(\),/:-\?\[-\]\{\}]*[\s\x0b]*=[\s\x0b]*[^!\(\),/:-\?\[-\]\{\}]+);?)*)*~"],
    ],
  ],
  [
    'id' => 934101,
    ...
    'rule' => [
      ['w' => ['cookie', 'get', 'files_name'], 'rx' => "~(?:close|exists|fork|(?:ope|spaw)n|re(?:ad|quire)|w(?:atch|rite))[\s\x0b]*\(~"],
    ],
  ],
  [
    'id' => 934120,
  ...
  [
    ...
    'rule' => [
      ['w' => ['cookie', 'get', 'files_name'], 'rx' => "~@+\{[\s\x0b]*\[~"],
    ],
  ],
  [
    'id' => 934180,
