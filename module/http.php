...
function server_start(string $host, int $port, int $workers): void {
  static $os = null, $ev = null, $pcntl = null, $opcache = null;
  if (null === $pcntl) {
    global $server_cnf;
    $os = $server_cnf['os'] ?? 'unix';
    $pcntl = $server_cnf['ext_pcntl'] ?? false;
    $ev = $server_cnf['ext_ev'] ?? false;
    $opcache_c = opcache_get_status();
    $opcache = ($opcache_c && $opcache_c['opcache_enabled']);
  }
...
function server_run(string $host, int $port): void {
  ...
  if (!\is_resource($server)) throw new RuntimeException("Failed to bind {$host}:{$port}: {$errstr} ({$errno})");
  \stream_set_blocking($server, false);

  ($ev) ? loop_ev($server) : loop_select($server);
}

/**
 * @param resource $server
 */
function loop_ev(mixed $server): void {
  static $max_header_size = null, $server_idle = null, $waf_on = null;
  static $watchers = [];
  if (null === $max_header_size) {
    global $server_cnf;
    $max_header_size = (int)($server_cnf['max_header_size'] ?? 8192);
    $server_idle = (int)($server_cnf['idle_second'] ?? 10);
  ...
  $watchers['server'] = $loop->io($server, \Ev::READ, function (\EvIo $w) use ($server, $loop, &$watchers, $max_header_size, $server_idle, $waf_on): void {
    while (($client = @stream_socket_accept($server, 0)) !== false) {
      ...
      if (false !== $fd) {
        @socket_set_option($fd, SOL_TCP, TCP_NODELAY, 1);
        @socket_set_option($fd, SOL_SOCKET, SO_KEEPALIVE, 1);
      }

      $state = ['sock' => $client, 'buffer' => '', 'offset' => 0, 'timer'  => null,];
      $watcher = null;

      $watcher = $loop->io($client, \Ev::READ, function (\EvIo $cw) use (&$watchers, $max_header_size, $waf_on): void {
          $state = &$cw->data;
          $sock  = $state['sock'];
          ...
          while (true) {
            ...
            if (isset($req['__invalid'])) {
              drop_connection($sock, (int)$req['__invalid']);
      ...
      $timer = $loop->timer($server_idle, 0.0, function (\EvTimer $tw) use (&$watchers, &$watcher, $client): void {
        $watcher->stop();
      ...
      $state['timer'] = $timer;
      $watcher->data = $state;
      $watchers[(int)$client] = ['io' => $watcher, 'timer' => $timer];
    }
  });
      ...
      if ($sock === $server) {
        ...
        if (false !== $client) {
          ...
          if (false !== $fd) {
            @socket_set_option($fd, SOL_TCP, TCP_NODELAY, 1);
            @socket_set_option($fd, SOL_SOCKET, SO_KEEPALIVE, 1);
            @socket_set_option($fd, SOL_SOCKET, SO_RCVBUF, 262144);
            @socket_set_option($fd, SOL_SOCKET, SO_SNDBUF, 262144);
          }
...
function parse_request(string &$buffer, int &$offset): ?array {
  ...
  if (3 !== \count($parts)) return ['__invalid' => 400];
  
  [$method, $uri, $http] = $parts;
  $method = \strtoupper($method);
  
  if (!isset($allowed[$method])) return ['__invalid' => 405];
  if (\strlen($uri) > $max_uri_length || false !== \strpos($uri, "\0")) return ['__invalid' => 414];
  if ('HTTP/1.1' !== $http && 'HTTP/1.0' !== $http) return ['__invalid' => 400];
  ...
  while ($h_start < $header_end - 2) {
    ...
    if (false === $colon) return ['__invalid' => 400];
    ...
    $headers[$key] = (isset($headers[$key])) ? ",{$val}" : $val;
...
function handle_req($sock, array $req, array $state): void {
  ...
  if ('head' !== $req['r_mtd']) $response .= $body;
...
function cpu_count(): int {
  ...
  if (0 === \stripos(PHP_OS, 'WIN')) {
    ...
    if ($res && \is_numeric($res)) $rt = (int) $res;
  }
  return $rt;
}
  ...
  if (isset($req['r_uri']) && \is_string($req['r_uri'])) {
    ...
    foreach ($route['mw'] ?? [] as $mw) {
      if (\is_string($mw) && \function_exists($mw)) {
        if (false === $mw(['req' => $req])) return gui_abort_html(401);
      }
    }
